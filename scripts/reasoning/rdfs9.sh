...
for p2 in $PREM2;
do
python main.py --knowledge $KNOW$p1$p2 --model_class 'bert' --model_path $B --template " is a particular"  --multi_token_handler "mean" --mode $m --multi_mask "m" --use_cuda True --trained_soft $s --is_premise True --save_path "result1208.csv"
done
done
done
done
...
for p2 in $PREM2;
do
python main.py --knowledge $KNOW$p1$p2 --model_class 'roberta' --model_path $B --template " is a particular"  --multi_token_handler "mean" --mode $m --multi_mask "m" --use_cuda True --trained_soft $s --is_premise True --save_path "result1208.csv"
done
done
done
done
